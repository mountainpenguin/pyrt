...
$(document).ready(function(){var a=window.location.pathname;StatSocket=new WebSocket(socket_protocol+"://"+window.location.host+a.substring(0,a.lastIndexOf("/")+1)+"statsocket");StatSocket.onmessage=onMessage;StatSocket.onopen=onOpen;StatSocket.onclose=onClose;StatSocket.onerror=onError});function mainLoop(){StatSocket.send("request=global");setTimeout(mainLoop,1E3)}
...
function onMessage(a){-1!==a.data.indexOf("ERROR")?$("#status-div").removeClass("status-ok status-bad").addClass("status-bad").html("StatSocket ERROR: "+a.data.split("ERROR/")[1]):(data=JSON.parse(a.data),"trackers"==data.type?(trackerData=data.data,initTGraph()):"global"==data.type&&($("#status-uprate").html("<div class='status-label status-system'>Upload rate:</div><div class='status-uprate-value status-value'>"+data.uprate_str+"/s</div>"),$("#status-downrate").html("<div class='status-label status-system'>Download rate:</div><div class='status-downrate-value status-value'>"+
