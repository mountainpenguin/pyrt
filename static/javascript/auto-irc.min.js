...
$("#dialog_form").dialog("open")})});function runSocketInit(){socket.send("request=get_sources")}function runSocketPostInit(){$(".bot_button").live("click",function(){if(-1!==$(this).attr("id").indexOf("start_")){var b=$(this).attr("id").split("start_")[1];socket.send("request=start_bot&arguments="+b)}else b=$(this).attr("id").split("stop_")[1],socket.send("request=stop_bot&arguments="+b)})}function onOpen(b){console.log("autoSocket opened",b,socket);runSocketInit()}
function onMessage(b){if("pong"!=b.data)if(0===b.data.indexOf("ERROR"))console.log(b.data),socket.close();else{var a=JSON.parse(b.data);if("get_sources"==a.request){if(a.error)return console.log("ERROR in request "+a.request+": "+a.error),!1;$("div#available").remove();b=$("table#available > tbody");for(i=0;i<a.response.length;i++)b.append($(a.response[i].row)),b.append($(a.response[i].req_row));runSocketPostInit()}else if("get_source_single"==a.request){if(a.error)return console.log("ERROR in request "+
a.request+": "+a.error),!1;refresh_drop_down_respond(a.response)}else if("add_filter"==a.request){if(a.error)return console.log("ERROR in request "+a.request+": "+a.error),!1;refresh_drop_down(a.name)}else if("start_bot"==a.request){if(a.error)return console.log("ERROR in request "+a.request+": "+a.error),refresh_drop_down(a.name),!1;setTimeout(function(){refresh_drop_down(a.name)},2E3)}else if("stop_bot"==a.request)a.error&&console.log("ERROR in request "+a.request+": "+a.error),refresh_drop_down(a.name);
else if("remove_filter"==a.request){if(a.error)return console.log("ERROR in request "+a.request+": "+a.error),!1;refresh_drop_down(a.name)}else if("set_source"==a.request){if(a.error)return console.log("ERROR in request "+a.request+": "+a.error),!1;refresh_drop_down(a.name)}else console.log("socket message:",b.data)}}function refresh_drop_down(b){socket.send("request=get_source_single&arguments="+b)}
function refresh_drop_down_respond(b){b=$(b.req_row);var a=b.attr("id").split("remote_settings_")[1];0==$(".bot_button",b).attr("id").indexOf("start_")?$(".status-"+a).html("off").removeClass("status-on").addClass("status-off"):$(".status-"+a).html("on").removeClass("status-off").addClass("status-on");$("#remote_settings_"+a).html(b.html())};
