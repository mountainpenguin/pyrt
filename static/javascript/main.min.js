...
$.inArray(torrent_id,DROP_OPEN)?drop_down(this):drop_up(torrent_id))});$(".batch-control").live("click",function(a){action=this.id.split("batch-")[1];torrentIDs=[];for(i=0;i<SELECTED.length;i++)torrentIDs.push(SELECTED[i].split("torrent_id_")[1]);if("delete"==action||"remove"==action)SELECTED=[],destroyBatchActionBox();sendme="request="+action+"_batch";sendme=sendme+"&torrentIDs="+torrentIDs.join(",");ws.send(sendme)});$("#batch-deselect").live("click",function(a){for(i=0;i<SELECTED.length;i++)torrent=
$("#"+SELECTED[i]),torrent.css({"background-color":""}),torrent.removeClass("ui-selected ui-selecting");SELECTED=[];destroyBatchActionBox()});$("#torrent_table").live("selectableselected",function(a,b){select_group_torrent(b.selected,a)}).live("selectableunselected",function(a,b){deselect_group_torrent(b.unselected,a)}).live("selectableselecting",function(a,b){-1==SELECTED.indexOf(b.selecting.id)&&$(b.selecting).css({"background-color":"#d9ffd9"})}).live("selectableunselecting",function(a,b){-1==
SELECTED.indexOf(b.unselecting.id)&&$(b.unselecting).css({"background-color":""})});$(".control_start_me").live("click",function(a){torrent_id=$(this).parents(".torrent-div").attr("id").split("torrent_id_")[1];command("start_torrent",torrent_id);return!1});$(".control_pause_me").live("click",function(a,b){torrent_id=$(this).parents(".torrent-div").attr("id").split("torrent_id_")[1];command("pause_torrent",torrent_id);return!1});$(".control_stop_me").live("click",function(a){torrent_id=$(this).parents(".torrent-div").attr("id").split("torrent_id_")[1];
command("stop_torrent",torrent_id);return!1});$(".control_remove_me").live("click",function(a){torrent_id=$(this).parents(".torrent-div").attr("id").split("torrent_id_")[1];command("remove_torrent",torrent_id);return!1});$(".control_delete_me").live("click",function(a){torrent_id=$(this).parents(".torrent-div").attr("id").split("torrent_id_")[1];command("delete_torrent",torrent_id);return!1});$(".download.allowed").live("click",function(a){a=$(this).next().next().text();var b=$(this).closest(".drop_down_container").attr("id").split("drop_down_container_")[1];
ws.send("request=download_file&torrentID="+b+"&path="+encodeURIComponent(a))})});function select_group_torrent(a,b){sel_index=SELECTED.indexOf(a.id);-1===sel_index&&(SELECTED.push(a.id),a.style.backgroundColor="#AEC798",1==SELECTED.length&&createBatchActionBox(b.pageX,b.pageY))}function deselect_group_torrent(a,b){sel_index=SELECTED.indexOf(a.id);-1!=sel_index&&(SELECTED.splice(sel_index,1),$(a).css({"background-color":""}),0==SELECTED.length&&destroyBatchActionBox())}
...
function command(a,b){if(-1!=$.inArray(b,DELETING))return!1;if("pause_torrent"===a||"start_torrent"===a||"stop_torrent"===a||"remove_torrent"==a||"delete_torrent"==a||"hash_torrent"==a){var c;if("remove_torrent"===a)c=confirm("Are you sure you want to remove this torrent?");else if("delete_torrent"==a){if(c=confirm("Are you sure you want to remove this torrent and *permanently* delete its files?"))$("#torrent_id_"+b).addClass("deleting-torrent-row").removeClass("blue green"),DELETING.push(b)}else c=
...
function messageHandler(a){if("pong"!=a.data){d=JSON.parse(a.data);if("get_info_multi"==d.request)return parse_content(d.response,"yes");"get_refresh_rate"==d.request?refresh_rate=1E3*parseInt(d.response):"get_torrent_info"==d.request?(newcellcontents=$(d.response),a=$(".drop_down_main",newcellcontents).first().attr("id").split("drop_down_main_")[1],a=document.getElementById("newcell_torrent_id_"+a),a.className+="drop_down_td",newcellcontents=accordionise(newcellcontents,1),newcellcontents=filetreeise(newcellcontents,
torrent_id),newcellcontents.css({display:"none"}),$(a).html(newcellcontents),newcellcontents.slideDown("slow")):"pause_torrent"==d.request||"start_torrent"===d.request||"stop_torrent"===d.request||"remove_torrent"==d.request||"delete_torrent"==d.request||"hash_torrent"==d.request?(a=d.response.trim(),"OK"!=a&&console.log("Command Failed with reason: "+a)):"download_file"==d.request?d.error?alert("Error in download handler: "+d.error):(a=d.response,console.log("Authkey is: "+a),window.open("download?auth="+
a,"downloadPage")):console.log("Unknown request")}};
