        ...
        for file in files:
            ...
            if file.base_path == rtorrent_root:
                ...
                if len(file.path_components) == 1:
                    ...
                    if os.path.basename(file.base_path) not in folder.keys():
                        folder[os.path.basename(file.base_path)] = {file.path_components[0] : {"_files" : [random_id], "_size" : file.size, "_priority" : [file.priority], "_completion" : file.percentage_complete}, "_files" : []}
                    else:
