...
class WebSocket(object):
    ...
    def __init__(self, get_mask_key = None):
        ...
        self.io_sock = self.sock = socket.socket(socket.AF_UNIX, socket.SOCK_STREAM)
    ...
    def connect(self, file_, **options):
        """
        ...
        """
        self.sock.connect(file_)
        self._handshake(**options)

    def _handshake(self, **options):
        sock = self.io_sock
        headers = []
        headers.append("GET /RPCSocket HTTP/1.1")
        headers.append("Upgrade: websocket")
        headers.append("Connection: Upgrade")
        headers.append("Host: None")
        headers.append("Origin: None")
...
if __name__ == "__main__":
    enableTrace(True)
    ws = create_connection("../.sockets/rpc.interface")
